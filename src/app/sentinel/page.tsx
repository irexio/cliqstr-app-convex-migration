...
export default function SentinelDashboard() {
  ...
  // Handle sentinel login
  const handleSentinelLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      if (authCheckRes.ok) {
        ...
        if (next.user?.account?.role !== 'Admin') {
          setError('You do not have sentinel privileges');
          return;
        }
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Loading sentinel dashboard...</h1>
      ...
      <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold">🛡️ Sentinel Access</h1>
            <p className="text-gray-600 mt-2">Please sign in with your sentinel credentials</p>
          </div>
          ...
          <form onSubmit={handleSentinelLogin}>
            <div className="mb-4">
  ...
  return (
    <main className="max-w-7xl mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-[#202020]">🛡️ Sentinel Dashboard</h1>
        <div className="text-sm text-gray-500">
          Logged in as {authStatus?.user?.profile?.username || authStatus?.user?.email} (Sentinel)
        </div>
      </div>
