...
export async function POST(req: Request) {
  try {
    ...
    const profileId = await convexHttp.mutation(api.profiles.createProfile, {
      userId: userId,
      username: username,
      birthdate: Date.now() - (15 * 365 * 24 * 60 * 60 * 1000), // Default to 15 years old (parental control age)
      firstName: username,
      lastName: '',
      about: '',
      image: '',
    ...
    const accountId = await convexHttp.mutation(api.accounts.createAccount, {
      userId: userId,
      birthdate: Date.now() - (15 * 365 * 24 * 60 * 60 * 1000), // Default to 15 years old (parental control age)
      role: 'Child',
      isApproved: false,
