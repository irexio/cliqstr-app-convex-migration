  ...
  try {
    ...
    if (user.account?.role === 'Child') {
      ...
      if (childSettings.inviteRequiresApproval) {
        ...
        const inviteRequestId = await convexHttp.mutation(api.invites.createInviteRequest, {
          childId: user.id as any,
          cliqId: cliqId as any,
          inviteeEmail: inviteType === 'child' ? trustedAdultContact : inviteeEmail,
    ...
    if (inviteType === 'child') {
      await sendChildInviteEmail({
        ...
        inviteLink: `${BASE_URL}/accept-invite?code=${inviteCode}`,
        inviteCode,
        inviteNote,
      });
    } else {
